...
void RKTransmitNextUserCommandChunk(unsigned char *buf, int buflen) {
	...
	if (reached_newline || reached_eof) {
		...
		const QString id = QString::number(RKRBackend::this_pointer->current_command->id);
		RKRBackend::this_pointer->handleOutput(id, id.length(), ROutput::CommandLineIn, false);
	}
}
